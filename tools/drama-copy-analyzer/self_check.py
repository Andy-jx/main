...
def run_checks(include_docs: bool = True, verbose: bool = True) -> None:
    ...
    first_line = plain_structure.splitlines()[0]
    ...
    if verbose and sys.stdout is not None:
        ...
        print("2) B 纯背景首段不强制标钩子：PASS")
        print("3) G 抽取本文人物/冲突/反转：PASS")
        print("4) H 每条引用原句：PASS")
        print("5) 新文案结果会变化：PASS")
        print("6) MD/TXT 报告八模块：PASS")
        print("7) 高频词重复提取：PASS")
        if include_docs:
            print("8) README / 买家说明 / 一键出包脚本：PASS")
